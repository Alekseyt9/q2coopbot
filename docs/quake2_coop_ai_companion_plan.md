...
### UDP harness evidence — 2026-09-22
...
В `base2` harness-прогон `coop-combat-531` подтвердил `monsters=41`,
реальный human player entity, входящий урон от монстров, выбор целей ботом
(`target_acquired` для monster entities `309` и `307`), 13 bot-side Blaster
...
fixture в 20/20 seed. LOS-механика также закрыта map fixture в 20/20, но
natural lost-LOS даёт только 16/20. Отдельная natural default-radius серия
`1312..1331` подтвердила 20/20 живых UDP-клиентов, но дала 0 yield.

### UDP baseline series — 2026-09-22

Через скрытый batch-раннер выполнены чистые серии с отдельными seed и
`minimumplayers 2`:

| Сценарий | Серия | Результат |
| --- | --- | --- |
| Combat | `coop-combat-538..557` | `20/20` UDP+human gate; `20/20` bot-owned fire; `6/20` эпизодов с подтверждённым bot damage по monster, всего `7` hit / `70` damage |
| Follow-style | `coop-follow-600..619` | `20/20` UDP+human gate; `0` stuck/regroup; дистанция не вышла за текущий scripted leash, поэтому полноценный follow acceptance не закрыт |
...
| Kill-steal natural default probe | `coop-kill-steal-default-1312..1331` (+ melee spot-check `1377..1380`) | `20/20` UDP+human gate; исходная natural default серия дала `134` player-focus и `0` yield; melee spot-check дал `2/4` strict yield (`1377/1378`), в `1379/1380` bot умер до yield; natural acceptance не закрыт |
...
| Live elevator route probe | `coop-elevator-natural-route-1348..1372` | `19/19` UDP+human; strict bot elevator acceptance прошёл в `2/19` seed (`1348`, `1358`); latest route revision достиг `31/33` и дал live player vertical rise в `1368/1369`, но bot не успел/не смог reacquire; health-guard не является position fixture; natural acceptance остаётся открытым |
...
192; естественная серия `1312..1331` тоже дала 0 yield при `20/20` живых
UDP-клиентах и требует map-aware timeline. Для lost-LOS добавлен отдельный strict edge `target_los_lost`, но
естественная серия `1292..1311` дала `16/20` эпизодов, поэтому нужна
conditional timeline для стабильного natural acceptance; отдельный map fixture
`1272..1291` закрыл саму LOS-механику в `20/20`. End-level transition и проверяемая persistence
закрыты отдельной strict UDP-серией; scripted waypoint для гарантированных
...
`coop-elevator-natural-route-1348..1372` уже дал natural
`TRAVEL_ELEVATOR`/reacquire acceptance в `2/19` seed (`1348`, `1358`).
Текущая версия маршрута отдельно доказала live player ride в `1368/1369`,
но bot-reacquire остаётся нестабильным; health guard меняет только живучесть
bot/player и не задаёт координаты.
