...
func TestPointPVSUsesObserverRowAndRejectsMalformedRows(t *testing.T) {
	...
	if possible, known := m.PointPVS(a, b); !known || possible {
		t.Fatal("PHS or reversed PVS row used")
	}
	if possible, known := m.PointPVS(b, a); !known || !possible {
		t.Fatal("asymmetric observer row ignored")
	}
	m.visibility.data[20] = 0
	m.visibility.data[21] = 0
	if _, known := m.PointPVS(a, b); known {
		t.Fatal("zero length RLE run accepted")
	}
	if _, known := (&MapInfo{}).PointPVS(a, b); known {
		t.Fatal("missing VIS treated as known")
	}
}
