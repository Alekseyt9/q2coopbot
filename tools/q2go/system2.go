...
func (s *Strategist) tick(w World) {
	...
	go func() {
		...
		requestJSON, _ := json.Marshal(map[string]any{"model": s.model, "prompt": prompt, "stream": false, "think": false, "format": "json", "options": map[string]any{"temperature": 0.1, "num_predict": 100}})
