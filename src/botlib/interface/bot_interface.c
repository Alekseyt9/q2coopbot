...
static int BotAI_CoopMode(void)
{
...
=============
*/
static float BotAI_CoopDangerScore(const bot_client_state_t *state)
{
	const int *inventory;
	float score = 0.0f;
	float health_score;
...
=============
*/
static bool BotAI_CoopDangerRequiresRetreat(const bot_client_state_t *state)
{
	float threshold;
...
static void BotInterface_DumpCoopMapModel(void)
{
		...
		entity = entity->next)
	{
		...
		BotLib_LogWrite(
			...
			"lip=%.1f spawnflags=%d",
			classname,
			AAS_ValueForBSPEpairKey(entity, "model") != NULL
				? AAS_ValueForBSPEpairKey(entity, "model") : "",
...
Observe the companion's current AAS area once per AI frame.  The state is
intentionally small; the complete adjacency remains in the opt-in map dump.
=============
*/
...
static void BotAI_CoopClearActionCommitment(bot_client_state_t *state)
{
	if (state == NULL)
	{
		return;
...
static void BotAI_CoopStartAction(bot_client_state_t *state,
	...
	bot_input_t *input)
{
	...
	needs_regroup = hard_leash > 0.0f &&
		(distance > hard_leash ||
			(bot_area > 0 && current_player_area > 0 &&
				bot_area != current_player_area &&
		...
		(result.failure || result.blocked))
	{
		...
		{
			BotLib_LogWriteTimeStamped(
				"coopbot_elevator_retry client=%d player=%d failure=%d blocked=%d",
...
=============
*/
static bool BotAI_ApplyCoopDirectionalMove(bot_client_state_t *state,
	float thinktime,
	bot_input_t *input,
	const vec3_t direction,
...
=============
*/
static bool BotAI_ApplyCoopCoverMove(bot_client_state_t *state,
	float thinktime,
	bot_input_t *input,
	const vec3_t direction,
	bool preserve_actions)
...
=============
*/
static bool BotAI_ApplyCoopBasicCover(bot_client_state_t *state,
	float thinktime,
	bot_input_t *input)
{
	vec3_t enemy_delta;
...
=============
*/
static bool BotAI_ApplyCoopFirelineAvoidance(bot_client_state_t *state,
	float thinktime,
	bot_input_t *input)
{
	vec3_t player_origin;
...
=============
*/
static bool BotAI_ApplyCoopDoorwayAvoidance(bot_client_state_t *state,
	float thinktime,
	bot_input_t *input)
{
	vec3_t player_origin;
...
=============
*/
static bool BotAI_ApplyCoopPersonalSpace(bot_client_state_t *state,
	float thinktime,
	bot_input_t *input)
{
	vec3_t player_origin;
