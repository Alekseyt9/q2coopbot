...
qboolean BotServerCmd(char *cmd, edict_t *ent, int server)
{
	...
	else if (Q_stricmp(cmd, "coopbot") == 0)
	{
		if (!gi.cvar("coop", "0", 0)->value)
		{
			gi.cprintf(ent, PRINT_HIGH, "coopbot requires coop 1\n");
		}
		else if (ent && (gi.cvar("serveronlybotcmds", "0", 0))->value)
		{
			gi.cprintf(ent, PRINT_HIGH, "not allowed to add bots\n");
		}
		else
