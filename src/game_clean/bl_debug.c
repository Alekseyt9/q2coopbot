...
#define COOPBOT_MAX_DEBUG_BOXES 2048
static visiblebbox_t coopbot_debug_boxes[COOPBOT_MAX_DEBUG_BOXES];

static visiblebbox_t *CoopBotDebugBox(edict_t *ent)
{
	ptrdiff_t index;
	...
	index = ent - g_edicts;
	if (index < 0 || index >= COOPBOT_MAX_DEBUG_BOXES)
		return NULL;

	return &coopbot_debug_boxes[index];
}
...
void ToggleVisibleBoundingBox(edict_t *ent)
{
	...
	if (ent->flags & FL_OBSERVER)
	{
		...
		if (cam != ent)
		{
			box = CoopBotDebugBox(cam);
			if (!box)
				return;
			if (box->created) DestroyVisibleBoundingBox(box);
			else CreateVisibleBoundingBox(box);
			return;
		} //end if
	} //end if
#endif
