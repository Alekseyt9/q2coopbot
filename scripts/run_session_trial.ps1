...
foreach($key in $cfg.PSObject.Properties.Name) {if($key -notin @('session','runtime_root','port','timescale')) {throw "Unknown session trial field: $key"}}
...
$source=(Resolve-Path -LiteralPath (Join-Path $base $cfg.runtime_root)).Path
$sessionPath=(Resolve-Path -LiteralPath (Join-Path $base $cfg.session)).Path
...
foreach($role in @('actor','observer')) {
    ...
    if($role -eq 'observer') {$test.scenario_tail_frames=5}
...
try {
    ...
    @{session='session.json';actor_trace='actor.jsonl';bot_trace='observer.jsonl';server_log='server.log';output='report.json'} | ConvertTo-Json | Set-Content -LiteralPath $reportConfig -Encoding utf8
